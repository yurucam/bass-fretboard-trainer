...
export function adaptToBassNoteFrequency(
  ...
  // 감지된 음은 실제 소리 그대로 반환 (옥타브 변환 없음)
  // 베이스 악보 관습은 매칭 로직에서 처리
  const actualSoundNote = step + accidental + octave;

  return {
    note: actualSoundNote, // 실제 감지된 소리 (E1은 E1 그대로)
